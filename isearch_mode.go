...
func init_isearch_mode(g *gemacs, backward bool) *isearch_mode {
	v := g.active.leaf
	m := new(isearch_mode)
...
func (m *isearch_mode) on_key(ev *termbox.Event) {
	...
	m.gemacs.isearch_last_word = copy_byte_slice(m.gemacs.isearch_last_word, new_word)
	m.search(false)
}
